...
def evaluate(segment_train_images, segment_validation_images, segment_train_labels, segment_validation_labels,test_index ):
    ...
    # model.add( PReLU())
    # model.add(Dropout(0.5))
    model.add(Flatten( input_shape=(1, sizeH, sizeV, sizeD),))
...
for train_index, test_index in loo.split(segment_training_set):
    ...
    print(test_index)
    val_acc,val_label,pred_label\
        = evaluate(segment_training_set[train_index], segment_training_set[test_index],segment_traininglabels[train_index], segment_traininglabels[test_index] ,test_index)
    tot+=val_acc
    accs.append(val_acc)
